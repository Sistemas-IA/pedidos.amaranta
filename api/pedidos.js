  ...
  try {
    ...
    if (req.method === "GET" && route === "ui-config") {
      ...
      return ok(res, {
        ...
        PAY_ALIAS_DETAIL: String((kv.PAY_ALIAS_DETAIL ?? "") ?? "").trim(),
    ...
    if (req.method === "POST" && route === "pedido") {
      ...
      if (!isOperator) {
        ...
        if (hasUpstash()) {
          ...
          if (!lock.ok) {
            ...
            const payAliasDetail = String(kv.PAY_ALIAS_DETAIL || "").trim();
      ...
      try {
        ...
        return ok(res, { idPedido, total, formaPago, zonaMensaje, payAlias, payAliasDetail, payNote });
      } catch (e) {
