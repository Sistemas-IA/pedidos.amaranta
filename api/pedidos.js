...
function formatReceiptDateBA(dateLike) {
...
function buildReceiptPayload({ dni, idPedido, fecha, formaPago, zonaMensaje = "", payAlias = "", payNote = "", items = [], total = 0 }) {
  return {
    dni: String(dni ?? "").trim(),
    idPedido: String(idPedido ?? "").trim(),
    fecha: formatReceiptDateBA(fecha),
    formaPago: String(formaPago ?? "").trim(),
    zonaMensaje: String(zonaMensaje ?? "").trim(),
    payAlias: String(payAlias ?? "").trim(),
    payNote: String(payNote ?? "").trim(),
    items: Array.isArray(items)
      ? items.map((it) => ({
          nombre: String(it?.nombre ?? "").trim(),
          cantidad: parseInt(String(it?.cantidad ?? 0), 10) || 0,
          precio: toNumOrNull(it?.precio) ?? 0,
        }))
      : [],
    total: toNumOrNull(total) ?? 0,
  };
}
...
async function findExistingOrderTodayByDni(dni){
  ...
  const fecha = tsFound || new Date().toISOString();

  return { dni: dniN, idPedido, items, total, fecha: formatReceiptDateBA(fecha), formaPago: String(formaPago || "").trim() };
}
    ...
    if (req.method === "POST" && route === "pedido") {
      ...
      try {
        ...
        for (const row of rowsV) {
          ...
          map.set(id, { nombre: row[1], precioTransf, precioEfectivo });
        }
        ...
        return ok(res, { idPedido, total, formaPago, zonaMensaje, payAlias, payNote, receipt });
      } catch (e) {
