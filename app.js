(() => {
  ...
  function normalizeReceiptOrder(raw){
    if (!raw || typeof raw !== "object") return null;
    return {
      dni: String(raw.dni ?? "").trim(),
      idPedido: String(raw.idPedido ?? "").trim(),
      fecha: String(raw.fecha ?? "").trim(),
      formaPago: String(raw.formaPago ?? "").trim(),
      zonaMensaje: String(raw.zonaMensaje ?? "").trim(),
      payAlias: String(raw.payAlias ?? "").trim(),
      payNote: String(raw.payNote ?? "").trim(),
  ...
  async function enviarPedido(){
    ...
    try {
      ...
      const order = normalizeReceiptOrder(data.receipt) || {
        dni,
      ...
      if (e?.code === "DNI_ALREADY_ORDERED" && e?.data?.existingOrder) {
        ...
        const order = normalizeReceiptOrder(e?.data?.receipt) || normalizeReceiptOrder(ex) || (ex && ex.idPedido ? {
          dni: ex.dni || dni,
