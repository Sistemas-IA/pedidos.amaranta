(() => {
  ...
  function getCartTtlMin(){
    const v = Number(state.config.CART_TTL_MIN ?? 45);
  ...
  function saveCart(){
    try {
      ...
      localStorage.setItem(CART_TS_KEY, String(state.cartTouchedAt || Date.now()));
    } catch {}
  }

  function loadCartIfFresh(){
    try {
      const raw = localStorage.getItem(CART_KEY);
      const ts = Number(localStorage.getItem(CART_TS_KEY) || 0);
      if (!raw) return;
      const age = Date.now() - (ts || 0);
      if (ts && age > getCartTtlMin() * 60 * 1000) {
        clearStoredCart();
        return;
      }
      ...
      for (const it of arr) {
        if (!it) continue;
        const id = it.id || it.IdVianda;
        if (!id) continue;
        const nombre = String(it.nombre || it.Nombre || "").trim();
        if (!nombre) continue;
        const cantidad = Number(it.cantidad || it.Cantidad || 0);
        if (!cantidad) continue;
        const precio = Number(it.precio || it.Precio || 0);
        state.cart.set(id, { id, nombre, precio, cantidad });
      }
      state.cartTouchedAt = ts || Date.now();
    } catch {}
  }
  ...
  function checkCartExpiry(silent=false){
    if (state.cart.size === 0) return;
    const ts = state.cartTouchedAt || Number(localStorage.getItem(CART_TS_KEY) || 0) || 0;
    if (!ts) return;
  ...
  async function refreshConfigOnly(){
    ...
    state.config = {
      ...
      CART_TTL_MIN: Number(conf.CART_TTL_MIN || 45),
