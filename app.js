(() => {
  ...
  const els = {
    ...
    tktPayDetail: document.getElementById("tkt-pay-detail"),
  ...
  async function refreshConfigOnly(){
    ...
    state.config = {
      ...
      PAY_ALIAS_DETAIL: String(conf.PAY_ALIAS_DETAIL || "").trim(),
  ...
  function formatAliasDetail(raw){
    ...
    if (parts.length >= 2) {
      return `Verificá antes de transferir:\nTitular: ${parts[0]}\nCUIL: ${parts.slice(1).join(" / ")}`;
    }
  ...
  function buildReceiptText(order){
    ...
    if (!efectivo) {
      // Seguridad: el mensaje de WhatsApp NO incluye alias ni datos de titular/CUIL.
      // Esos datos quedan solo visibles en el ticket para evitar activar malware de portapapeles.
  ...
  function openTicket(order){
    ...
    const aliasDetail = formatAliasDetail(order.payAliasDetail || state.config.PAY_ALIAS_DETAIL || "");
    if (els.tktPayDetail) {
      if (!efectivo && aliasDetail) {
        els.tktPayDetail.textContent = aliasDetail;
        els.tktPayDetail.classList.remove("hidden");
      } else {
        els.tktPayDetail.textContent = "";
        els.tktPayDetail.classList.add("hidden");
      }
    }
    ...
    if (efectivo) {
      ...
      els.tktNote.textContent = (!baseNoteAll || looksLikeWhatsAppInstruction)
        ? tip
        : `${baseNoteAll}\n\n${tip}`;
    }
      ...
      els.tktSave.onclick = () => {
        ...
        openAlertModal({
          ...
          msg:
            ...
            "Tip: sacale una captura de pantalla a este comprobante y enviala manualmente.",
        });
      };
    }
  }
  ...
  async function enviarPedido(){
    ...
    try {
      ...
      const order = {
        ...
        payAliasDetail: String((data.payAliasDetail || state.config.PAY_ALIAS_DETAIL || "")).trim(),
      ...
      if (e?.code === "DNI_ALREADY_ORDERED" && e?.data?.existingOrder) {
        ...
        if (ex && ex.idPedido){
          const order = {
            ...
            payAliasDetail: String(ex.payAliasDetail || state.config.PAY_ALIAS_DETAIL || "").trim(),
